...
export const getSets = () => (dispatch, getState) => {
  axios.get('/api/gastro/patho/', tokenConfig(getState))
    .then(res => {
      dispatch({
        type: GET_SETS,
        payload: res.data
...
export const addSet = set => (dispatch, getState) => {
  axios
    .post('/api/gastro/patho/', set, tokenConfig(getState), {
      headers: {
...
export const updateSet = (set, id) => (dispatch, getState) => {
  axios
    .put(`/api/gastro/patho/${id}/`, set, tokenConfig(getState), {
      headers: {
        'content-type': 'multipart/form-data'
      }
    })
...
export const deleteSet = (id) => (dispatch, getState) => {
  axios
   .delete(`/api/gastro/patho/${id}/`, tokenConfig(getState))
    .then(res => {
