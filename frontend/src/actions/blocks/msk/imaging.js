...
export const getSets = () => (dispatch, getState) => {
  axios.get('/api/msk/imaging/', tokenConfig(getState))
    .then(res => {
      dispatch({
        type: GET_SETS,
        payload: res.data
...
export const addSet = set => (dispatch, getState) => {
  axios
    .post('/api/msk/imaging/', set, tokenConfig(getState), {
      headers: {
...
export const updateSet = (set, id) => (dispatch, getState) => {
  axios
    .put(`/api/msk/imaging/${id}/`, set, tokenConfig(getState), {
      headers: {
        'content-type': 'multipart/form-data'
      }
    })
...
export const deleteSet = (id) => (dispatch, getState) => {
  axios
   .delete(`/api/msk/imaging/${id}/`, tokenConfig(getState))
    .then(res => {
