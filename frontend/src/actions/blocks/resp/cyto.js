...
export const getSets = () => (dispatch, getState) => {
  axios.get('/api/resp/cyto/', tokenConfig(getState))
    .then(res => {
      dispatch({
        type: GET_SETS,
        payload: res.data
...
export const addSet = set => (dispatch, getState) => {
  axios
    .post('/api/resp/cyto/', set, tokenConfig(getState), {
      headers: {
...
export const updateSet = (set, id) => (dispatch, getState) => {
  axios
    .put(`/api/resp/cyto/${id}/`, set, tokenConfig(getState), {
      headers: {
        'content-type': 'multipart/form-data'
      }
    })
...
export const deleteSet = (id) => (dispatch, getState) => {
  axios
   .delete(`/api/resp/cyto/${id}/`, tokenConfig(getState))
    .then(res => {
