...
import { getSets, deleteSet } from "../../../../actions/blocks/msk/patho";
import { Link } from "react-router-dom";


export class ListMSKP extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return (
      <div className="container">
        <h1 className="text-center py-2 text-info">Musculoskeletal: Pathology Sets</h1>
        <Link className="btn btn-secondary" to="/msk/">
          Previous Page
        </Link>
        {user ? this.props.auth.user.profile.role == 'Instructor' && (
        <Link className="btn btn-info ml-1" to="/msk/pathology/create/">
          Add a New Set
        </Link>
         ): ""}
        <p></p>
...
export default connect(mapStateToProps, { getSets, deleteSet })(ListMSKP);
