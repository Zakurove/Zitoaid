...
import { getSets, deleteSet } from "../../../../actions/blocks/msk/imaging";
import { Link } from "react-router-dom";


export class ListMSKI extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return (
      <div className="container">
        <h1 className="text-center py-2 text-info">Musculoskeletal: Imaging Sets</h1>
        <Link className="btn btn-secondary" to="/msk/">
          Previous Page
        </Link>
        {user ? this.props.auth.user.profile.role == 'Instructor' && (
        <Link className="btn btn-info ml-1" to="/msk/imaging/create/">
          Add a New Set
        </Link>
         ): ""}
        <p></p>
...
export default connect(mapStateToProps, { getSets, deleteSet })(ListMSKI);
