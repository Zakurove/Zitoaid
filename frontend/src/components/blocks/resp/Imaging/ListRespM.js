...
import { getSets, deleteSet } from "../../../../actions/blocks/resp/imaging";
import { Link } from "react-router-dom";


export class ListRespI extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return (
      <div className="container">
        <h1 className="text-center py-2 text-info">Respiratory: Imaging Sets</h1>
        <Link className="btn btn-secondary" to="/respiratory/">
          Previous Page
        </Link>
        {user ? this.props.auth.user.profile.role == 'Instructor' && (
        <Link className="btn btn-info ml-1" to="/respiratory/imaging/create/">
          Add a New Set
        </Link>
         ): ""}
        <p></p>
...
export default connect(mapStateToProps, { getSets, deleteSet })(ListRespI);
