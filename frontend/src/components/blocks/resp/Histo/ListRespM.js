...
import { getSets, deleteSet } from "../../../../actions/blocks/resp/histo";
import { Link } from "react-router-dom";


export class ListRespH extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return (
      <div className="container">
        <h1 className="text-center py-2 text-info">Respiratory: Histology Sets</h1>
        <Link className="btn btn-secondary" to="/respiratory/">
          Previous Page
        </Link>
        {user ? this.props.auth.user.profile.role == 'Instructor' && (
        <Link className="btn btn-info ml-1" to="/respiratory/histology/create/">
          Add a New Set
        </Link>
         ): ""}
        <p></p>
          ...
          <tbody>
            ...
            {this.props.sets.map((set) => (
              <tr key={set.id}>
                ...
                <td>
                  <Link
                    to={"/respiratory/histology/" + set.id}
...
export default connect(mapStateToProps, { getSets, deleteSet })(ListRespH);
