...
import { getSets, deleteSet } from "../../../../actions/blocks/resp/patho";
import { Link } from "react-router-dom";


export class ListRespP extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return (
      <div className="container">
        <h1 className="text-center py-2 text-info">Respiratory: Pathology Sets</h1>
        <Link className="btn btn-secondary" to="/respiratory/">
          Previous Page
        </Link>
        {user ? this.props.auth.user.profile.role == 'Instructor' && (
        <Link className="btn btn-info ml-1" to="/respiratory/pathology/create/">
          Add a New Set
        </Link>
         ): ""}
        <p></p>
          ...
          <tbody>
            ...
            {this.props.sets.map((set) => (
              <tr key={set.id}>
                ...
                <td>
                  <Link
                    to={"/respiratory/pathology/" + set.id}
...
export default connect(mapStateToProps, { getSets, deleteSet })(ListRespP);
