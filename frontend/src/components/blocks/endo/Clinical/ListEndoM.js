...
import { getSets, deleteSet } from "../../../../actions/blocks/endo/clinical";
import { Link } from "react-router-dom";


export class ListEndoCT extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return (
      <div className="container">
        <h1 className="text-center py-2 text-info">Endocrine: Clinical Sets</h1>
        <Link className="btn btn-secondary" to="/endocrine/">
          Previous Page
        </Link>
        {user ? this.props.auth.user.profile.role == 'Instructor' && (
        <Link className="btn btn-info ml-1" to="/endocrine/clinicalTests/create/">
          Add a New Set
        </Link>
         ): ""}
        <p></p>
          ...
          <tbody>
            ...
            {this.props.sets.map((set) => (
              <tr key={set.id}>
                ...
                <td>
                  <Link
                    to={"/endocrine/clinicalTests/" + set.id}
...
export default connect(mapStateToProps, { getSets, deleteSet })(ListEndoCT);
