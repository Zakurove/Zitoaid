...
import { getSets, deleteSet } from "../../../../actions/blocks/endo/cyto";
import { Link } from "react-router-dom";


export class ListEndoC extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return (
      <div className="container">
        <h1 className="text-center py-2 text-info">Endocrine: Cytology Sets</h1>
        <Link className="btn btn-secondary" to="/endocrine/">
          Previous Page
        </Link>
        {user ? this.props.auth.user.profile.role == 'Instructor' && (
        <Link className="btn btn-info ml-1" to="/endocrine/cytology/create/">
          Add a New Set
        </Link>
         ): ""}
        <p></p>
...
export default connect(mapStateToProps, { getSets, deleteSet })(ListEndoC);
